...
FastAPI service backing the ElderPay frontend: speech-to-text + intent parsing, the transaction state machine, face-based auth, and account enrollment. Storage is in-memory unless `DATABASE_URL` is set (Postgres); Paystack handles real NGN payout transfers when configured — built for the NITHUB Innovation Fair Hackathon 2026, not production.
...
| `/api/transactions/send` | POST | Execute a `FACE_VERIFIED` transaction; idempotent |
...
### Paystack payout onboarding + withdrawal
The POS agent's payout profile is shared across customers. `POST /api/agent/payout-onboard` takes `bankAccountNumber` and `bankCode`, resolves the account name, creates a Paystack NUBAN transfer recipient, and stores its recipient code. `GET /api/agent/payout-status` reports the current profile. An onboarded profile routes withdrawals through Paystack Transfers; otherwise withdrawals use the mock ledger.

### Health
`/api/health` — `{ok, demoMode, paystackConfigured, dbConnected}`
...
app/
  ...
  services/
    ...
    paystack_service.py       Paystack account resolution and transfers
    mock_ledger.py             Provider-neutral demo ledger helpers
...
- Paystack uses `httpx.AsyncClient` for account resolution and transfers. Real transfers require `PAYSTACK_SECRET_KEY` and completed business KYB; test keys exercise the request flow without moving real money. P2P send, deposits, and airtime remain on the local mock ledger.
